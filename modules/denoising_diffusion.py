...
class GaussianDiffusion(nn.Module):
    ...
    def compress(
        ...
        if self.ae_fn is None:
            return (
                self.p_sample_loop(
                    images.shape, context_dict["output_hat"], clip_denoised=True, init=init, eta=eta
                ),
    ...
    def p_losses(self, x_start, context_dict, t, aux_img=None):
        ...
        if self.denoise_fn.embd_type == "01":
            ...
            fx_hat = self.denoise_fn(
                x_noisy, t.float().unsqueeze(-1) / self.num_timesteps, context=context_dict["output_hat"]
            )
        else:
            fx = self.denoise_fn(
        ...
        if self.pred_mode == "noise":
            ...
            if self.loss_type == "l1":
                ...
                err = (err * weight).mean()
                err_hat = F.mse_loss(x_start, fx_hat, reduction='none').mean(dim=(1, 2, 3))
                err_hat = (err_hat * weight).mean()
            else:
                raise NotImplementedError()
        elif self.pred_mode == "v":
            if self.use_loss_weight:
        ...
        if self.aux_loss_weight > 0:
            ...
            if self.aux_loss_type == "l1":
                ...
                for i in range(aux_img.shape[0]):
                    ...
                    aux_err_hat.append(self.loss_fn_vgg(aux_img[i:i+1], pred_x0_hat[i:i+1]).mean())
                aux_err = torch.stack(aux_err, dim=0)
                aux_err = (weight * aux_err).mean()
                aux_err_hat = torch.stack(aux_err_hat, dim=0)
                aux_err_hat = (weight * aux_err_hat).mean()
            else:
                raise NotImplementedError()

            loss = (
                # self.lagrangian_beta * context_dict["bpp"].mean()
                # + (err + err_hat) * (1 - self.aux_loss_weight)
                # + (aux_err + aux_err_hat) * self.aux_loss_weight
                self.lagrangian_beta * context_dict["bpp"].mean()
                + (err_hat) * (1 - self.aux_loss_weight)
                + (aux_err_hat) * self.aux_loss_weight
            )
        else:
            loss = self.lagrangian_beta * context_dict["bpp"].mean() + err + err_hat
