...
class RateAdaptionEncoder(nn.Module):
    def __init__(self, channel_num, rate_choice, mode='CHW', step=2):
        ...
        masks = ()
        for i in range(step):
            maski = torch.arange((max(self.rate_choice)//self.step)*i, (max(self.rate_choice)//self.step)*(i+1)).unsqueeze(-1)
            masks = masks + (maski,)

        mask  = torch.cat(masks, dim=-1).view(-1).repeat(self.H * self.W, 1)
    ...
    def forward(self, x, indexes):
        ...
        # print(torch.sum(self.rate_choice_tensor[indexes]))
    ...
    def update_resolution(self, H, W, device):
        ...
        self.mask  = torch.cat(masks, dim=-1).view(-1).repeat(self.num_patches, 1)
...
class JSCCEncoder(nn.Module):  
    def __init__(self, embed_dim=256, depths=[3], input_resolution=(16, 16), 
                 num_heads=[8], window_size=16,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 norm_layer=nn.LayerNorm, rate_choice=[16, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256]):
        ...
        self.refine = Mlp(embed_dim, embed_dim * 8, embed_dim)
    ...
    def forward(self, x, px, eta):
        ...
        x_BLC = x.flatten(2).permute(0, 2, 1)
        x_BLC = x_BLC + self.refine(x_BLC)
        # https://vimsky.com/examples/usage/python-torch.searchsorted-pt.html
        indexes = torch.searchsorted(self.rate_choice_tensor, symbol_num).clamp(0, self.rate_num - 1)  # B*H*W
        
        # https://blog.csdn.net/devil_son1234/article/details/127994054
