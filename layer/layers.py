...
class RoPE(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        B, H, W, C = x.size()
        # x = x.permute().flatten(2).permute(0, 2, 1)
        if H != self.H or W != self.W:
            self.update_resolution(H, W, x, x.get_device())
    ...
    def update_resolution(self, H, W, x, device):
        self.H = H
        self.W = W

        channel_dims = (self.H, self.W)
        ...
        rotations_re = torch.cos(angles).unsqueeze(dim=-1)
        rotations_im = torch.sin(angles).unsqueeze(dim=-1)
        self.rotations = torch.cat([rotations_re, rotations_im], dim=-1).to(device)
...
class BasicLayerDec(nn.Module):
    ...
    def __init__(self, dim, out_dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            MLLABlock(dim=out_dim, input_resolution=input_resolution, num_heads=num_heads,
